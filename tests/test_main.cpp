...
std::string BuildDexCodeItem(const std::vector<std::uint16_t>& instructions) {
  return BuildDexCodeItem(instructions, 0u);
}

struct DexReferencedMethodFixture {
...
std::string BuildDexPayloadWithEntrypoint(
  ...
  auto append_unique_type = [&](const std::string& value) {
    if (std::find(type_descriptors.begin(), type_descriptors.end(), value) ==
        type_descriptors.end()) {
      type_descriptors.push_back(value);
    }
  };
  ...
  const std::uint32_t field_ids_off = method_ids_off + method_ids_size * 8u;
  const std::uint32_t class_defs_off = field_ids_off + field_ids_size * 8u;
  ...
  for (std::size_t index = 0; index < extra_method_references.size(); ++index) {
    const auto& method_reference = extra_method_references[index];
    const std::size_t offset = method_ids_off + (index + 1u) * 8u;
  ...
  for (std::size_t index = 0; index < extra_field_references.size(); ++index) {
    ...
    write_le16(offset + 0u,
               static_cast<std::uint16_t>(
                   type_indices.at(field_reference.class_descriptor)));
    write_le16(offset + 2u,
               static_cast<std::uint16_t>(
                   type_indices.at(field_reference.field_type_descriptor)));
  ...
  write_le32(80, field_ids_size);
  write_le32(84, field_ids_off);
...
std::string BuildFrameworkBoundaryLifecycleOnCreateDexPayload(
    const std::vector<std::string>& class_descriptors,
    const std::string& requested_entrypoint_class_descriptor) {
...
std::string BuildObjectFieldLifecycleOnCreateDexPayload(
  ...
  Expect(type_it != ordered_types.end(),
         "expected StateCarrier descriptor in object-field lifecycle fixture");
...
void TestLaunchApkReportsJniRegistrationCallbackDispatchOutcomePrecisely() {
  namespace fs = std::filesystem;
  const fs::path build_dir = ResolveBuildDirFromTestBinary();
  const fs::path compatctl = build_dir / "compatctl";
  const fs::path fixture_library =
  ...
  Expect(output.find(
             ...
             std::string::npos,
         "expected activity onCreate dispatch boundary after registration fixture");
  Expect(output.find(
             "\"native_loading_state\": "
             "\"activity_oncreate_bundle_dispatch_required\"") !=
             std::string::npos,
         "expected native loading state to preserve post-binding seam");
  Expect(output.find("\"native_jni_state\": \"called\"") !=
             std::string::npos,
  ...
  Expect(output.find(
             ...
             std::string::npos,
         "expected JNI registration callback dispatch state");
  Expect(output.find(
  ...
  Expect(output.find(
             ...
             "\"register_LinuxoidFixture\"") != std::string::npos,
         "expected registration callback symbol in launch json");
  Expect(output.find(
             "\"native_registration_outcome_state\": "
             "\"register_natives_completed\"") != std::string::npos,
         "expected RegisterNatives completion in launch json");
  Expect(output.find(
             "\"native_registration_class_name\": "
             "\"com/example/linuxoid/Fixture\"") != std::string::npos,
         "expected registered class name in launch json");
  Expect(output.find("\"native_registration_method_count\": 1") !=
             std::string::npos,
  ...
  Expect(output.find(
             ...
             "\"activity_oncreate_bundle_dispatch_required\"") !=
             std::string::npos,
         "expected precise post-binding activity dispatch state");
  Expect(output.find(
  ...
  Expect(output.find(
             ...
             "\"linuxoid_runtime_context_bound\"") != std::string::npos,
         "expected managed runtime context binding state in launch json");
  Expect(output.find(
             "\"native_managed_activity_runtime_binding_reason\": "
             "\"managed_activity_dispatch_target_bound_to_linuxoid_runtime_context\"") !=
             std::string::npos,
         "expected managed runtime context binding reason in launch json");
  Expect(output.find(
             "\"native_managed_activity_runtime_context_kind\": "
             "\"linuxoid_managed_runtime_context_placeholder\"") !=
             std::string::npos,
         "expected managed runtime context placeholder kind in launch json");
  Expect(output.find(
             "\"native_managed_activity_runtime_context_id\": "
             "\"linuxoid-runtime-context:com.example.launchapk:com.example.launchapk.MainActivity\"") !=
             std::string::npos,
         "expected managed runtime context id in launch json");
  Expect(output.find("\"native_loading_library_name\": \"libjni_latinime.so\"") !=
             std::string::npos,
  ...
  Expect(output.find(
             ...
             "\"activity_oncreate_bundle_dispatch_required\"") !=
             std::string::npos,
         "expected nested native execute post-binding activity dispatch state");
  Expect(output.find(
  ...
  Expect(output.find(
             ...
             "\"linuxoid_runtime_context_bound\"") != std::string::npos,
         "expected nested native execute runtime context binding state");

  fs::remove_all(fixture.root);
}

void TestLaunchApkFirstAppStartReportsPostRegistrationManagedDispatchBoundary() {
  namespace fs = std::filesystem;
  const fs::path build_dir = ResolveBuildDirFromTestBinary();
  const fs::path compatctl = build_dir / "compatctl";
  const fs::path fixture_library =
  ...
  Expect(output.find(
             "\"native_registration_dispatch_state\": \"called\"") !=
             std::string::npos,
         "expected registration callback dispatch state in first-app-start proof");
  Expect(output.find(
             "\"native_registration_outcome_state\": "
             "\"register_natives_completed\"") != std::string::npos,
  ...
  Expect(output.find(
             ...
             "\"activity_oncreate_bundle_dispatch_required\"") !=
             std::string::npos,
         "expected post-binding dispatch seam in first-app-start proof");
  Expect(output.find(
             "\"native_post_jni_dispatch_symbol\": "
             "\"com.example.launchapk.MainActivity->onCreate(Landroid/os/Bundle;)V\"") !=
             std::string::npos,
  ...
  Expect(output.find(
             "\"native_managed_activity_runtime_binding_state\": "
             "\"linuxoid_runtime_context_bound\"") != std::string::npos,
         "expected runtime context binding state in first-app-start proof");
  Expect(output.find(
             "\"native_managed_activity_runtime_context_kind\": "
             "\"linuxoid_managed_runtime_context_placeholder\"") !=
             std::string::npos,
...
void TestLaunchApkFirstAppStartCheckpointExecutesFirstDexInstruction() {
  ...
  Expect(output.find("\"framework_boundary_state\": \"framework-stubbed\"") !=
             std::string::npos,
         "expected stubbed framework boundary state in first app start json");
  Expect(output.find(
             "\"invoked_method_class_descriptor\": \"Landroid/app/Activity;\"") !=
             std::string::npos,
         "expected invoked framework class descriptor in first app start json");
  Expect(output.find("\"invoked_method_name\": \"onCreate\"") !=
             std::string::npos,
         "expected invoked framework method name in first app start json");
  Expect(output.find("\"invoked_method_signature\": \"()V\"") !=
             std::string::npos,
         "expected invoked framework method signature in first app start json");
  ...
  Expect(output.find("\"object_register_field_state\": "
         ...
         "expected placeholder object/register/field state in first app start json");
  Expect(output.find("\"object_class_descriptor\": "
                     "\"Lcom/example/launchapk/StateCarrier;\"") !=
             std::string::npos,
         "expected object class descriptor in first app start json");
  Expect(output.find("\"field_class_descriptor\": "
                     "\"Lcom/example/launchapk/StateCarrier;\"") !=
             std::string::npos,
