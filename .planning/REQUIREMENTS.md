...
- [ ] **JNI-13**: Linuxoid can bind a Linuxoid-owned managed runtime context to the selected launcher activity lifecycle target after JNI registration and managed dispatch target selection
- [ ] **JNI-14**: Linuxoid keeps managed runtime-context binding, lifecycle invocation attempt, and later framework bootstrap seams distinct in launch and first-app-start reporting
...
- [ ] **VER-09**: User can push the real verification APK one step past the current `managed_runtime_context_required` seam and see the first exact managed runtime-context, lifecycle dispatch, or framework bootstrap blocker
...
| JNI-13 | Phase 12 | Pending |
| JNI-14 | Phase 12 | Pending |
