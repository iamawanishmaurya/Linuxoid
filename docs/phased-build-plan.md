...
Current repo note as of `2026-05-18`: The next direct-runtime milestone after P14 is **P15 Managed Bytecode Invocation + ActivityThread Contract**. P15 should consume the existing sandbox-backed `package_manager`, `intent_resolution`, `activity_launch`, `storage`, `permissions`, `app_ops`, `activity_manager`, `process_manager`, `window_manager`, `runtime_bridge`, and `java_apk_proof` contracts plus the current Self-Healing Android Device health gates (`storage_health`, `sandbox_health`, `permission_health`, `app_ops_health`, `binder_health`, `dex_health`, `activity_health`, `activity_manager_health`, `process_health`, `window_health`, `runtime_health`, and `java_proof_health`) as its real managed-runtime-start inputs. What remains blocked after P14 is real ART-owned class loading, ActivityThread-style application bootstrap, managed bytecode invocation, richer framework-level permission/AppOps enforcement, and full Java/Kotlin execution.
