...
Current repo note as of `2026-05-18`: The next direct-runtime milestone after P12 is **P13 Runtime Process Handoff + Resume Contract**. P13 should consume the existing sandbox-backed `package_manager`, `intent_resolution`, `activity_launch`, `storage`, `permissions`, `app_ops`, `activity_manager`, `process_manager`, and `window_manager` contracts plus the current Self-Healing Android Device health gates (`storage_health`, `sandbox_health`, `permission_health`, `app_ops_health`, `binder_health`, `dex_health`, `activity_health`, `activity_manager_health`, `process_health`, and `window_health`) as its real runtime-start and runtime-resume inputs. What remains blocked after P12 is real host-side process ownership and liveness beyond the current placeholder process identity, richer framework-level permission/AppOps enforcement, real Android window/compositor semantics, and full Java/Kotlin ART execution.
