...
Current repo note as of `2026-05-17`: Linuxoid now also has a direct `compatctl launch-apk <apk-path> [staging-root]` slice for native-only APKs. It can inspect a stored ZIP APK, read plain-XML manifest metadata, stage ABI-matching native libraries plus assets into a deterministic session directory, `dlopen` the selected library, call `JNI_OnLoad` when present, create a minimal native activity/session object, and emit structured JSON. That slice is still intentionally narrow: binary Android manifest XML, Java/Kotlin startup, and ART-owned execution are still pending.
...
Current repo note as of `2026-05-18`: Linuxoid now also has a **First DEX App-Method Invocation Checkpoint** through `compatctl launch-apk --first-app-start-proof [--package <package>] [--component <component>] <apk-path> [staging-root]` and `compatctl inspect-apk-first-start <apk-path> [staging-root]`. That checkpoint reuses the real direct-session package, activity, process, window, runtime, and Java-proof seams, persists `sandbox/data/data/<package>/first-app-start/first-app-start.json`, parses real DEX string plus type plus proto plus field plus method plus class tables, and reports whether Linuxoid actually decoded and executed a real DEX instruction path or stopped at an exact unsupported or runtime-handoff boundary. The healthy fixture path now resolves `Lcom/example/launchapk/MainActivity;` from staged DEX metadata, materializes a deterministic lifecycle receiver placeholder in register `v0`, executes the deterministic `com.example.launchapk.MainActivity.onCreate()I` lifecycle method through Linuxoid's minimal interpreter, crosses a stubbed `Landroid/app/Activity;->onCreate()V` framework boundary through `invoke-super`, executes one tiny app-local `invoke-direct` helper `linuxoidComputeValue()I` with `move-result` propagation, allocates a placeholder `Lcom/example/launchapk/StateCarrier;` object, executes deterministic `iput` plus `iget` access for `value:I`, returns the deterministic integer value, and still reports `needs-real-activitythread-context` as the next honest blocker, but full ART-owned managed class loading, invocation, objects, and bytecode execution are still blocked.

Current repo note as of `2026-05-18`: The next direct-runtime milestone after this checkpoint is still **P16 Managed Bytecode Invocation + ActivityThread Contract**, but it should now start from a narrower concrete blocker: bridge the resolved `MainActivity`, the lifecycle receiver placeholder, the first executed app-local `linuxoidComputeValue()I` invoke-direct path, the stubbed `android.app.Activity.onCreate()V` boundary, and the placeholder `StateCarrier.value:I` object/field seam into a real ART-owned ActivityThread or equivalent managed app context. The existing sandbox-backed `package_manager`, `intent_resolution`, `activity_launch`, `storage`, `permissions`, `app_ops`, `activity_manager`, `process_manager`, `window_manager`, `runtime_bridge`, `java_apk_proof`, and `compatibility` contracts remain the real input seams. What remains blocked after this checkpoint is not DEX parsing anymore; it is real ART-owned class loading, managed receiver construction, managed app-local method invocation, real managed object identity and field state, ActivityThread-style application bootstrap, richer framework-level permission/AppOps enforcement, and full Java/Kotlin execution across broader third-party APKs.
