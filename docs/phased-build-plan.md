...
Current repo note as of `2026-05-18`: Linuxoid now also has a **First DEX Object/Register/Field Checkpoint** through `compatctl launch-apk --first-app-start-proof [--package <package>] [--component <component>] <apk-path> [staging-root]` and `compatctl inspect-apk-first-start <apk-path> [staging-root]`. That checkpoint reuses the real direct-session package, activity, process, window, runtime, and Java-proof seams, persists `sandbox/data/data/<package>/first-app-start/first-app-start.json`, parses real DEX string plus type plus proto plus field plus method plus class tables, and reports whether Linuxoid actually decoded and executed a real DEX instruction path or stopped at an exact unsupported or runtime-handoff boundary. The healthy fixture path now executes the deterministic `com.example.launchapk.MainActivity.onCreate()I` lifecycle method through Linuxoid's minimal interpreter, crosses a stubbed `Landroid/app/Activity;->onCreate()V` framework boundary through `invoke-super`, allocates a placeholder `Lcom/example/launchapk/StateCarrier;` object, executes deterministic `iput` plus `iget` access for `value:I`, returns the deterministic integer value, and still reports `needs-real-activitythread-context` as the next honest blocker, but full ART-owned managed objects and bytecode invocation are still blocked.

Current repo note as of `2026-05-18`: The next direct-runtime milestone after this checkpoint is still **P16 Managed Bytecode Invocation + ActivityThread Contract**, but it should now start from a narrower concrete blocker: bridge the resolved `MainActivity`, the first executed `onCreate()I` DEX path, the stubbed `android.app.Activity.onCreate()V` boundary, and the new placeholder `StateCarrier.value:I` object/field seam into a real ART-owned ActivityThread or equivalent managed app context. The existing sandbox-backed `package_manager`, `intent_resolution`, `activity_launch`, `storage`, `permissions`, `app_ops`, `activity_manager`, `process_manager`, `window_manager`, `runtime_bridge`, `java_apk_proof`, and `compatibility` contracts remain the real input seams. What remains blocked after this checkpoint is not DEX parsing anymore; it is real ART-owned managed method invocation, real managed object identity and field state, ActivityThread-style application bootstrap, richer framework-level permission/AppOps enforcement, and full Java/Kotlin execution across broader third-party APKs.
