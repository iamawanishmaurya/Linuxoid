...
Current repo note as of `2026-05-19`: Linuxoid now also has a sharpened **JNI and Native Loading Checkpoint** on that same real APK path. `compatctl launch-apk /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` now preloads the tiny Android soname compatibility shims only when staged libraries actually need them, normalizes undefined ELF version bindings when safe, and surfaces those facts through nested `native_execute.android_compat_state`, `elf_undefined_versions_normalized`, `android_compat_preloaded_paths`, and `android_compat_diagnostics`. Linuxoid also now defers `JNI_OnLoad` to the selected entry library instead of eagerly calling it on every loaded helper library, gets `libjni_latinime.so` loaded, calls `JNI_OnLoad`, and records deterministic `jni_onload_results`. On the real keyboard APK path that moves the blocker forward again from the earlier unresolved-symbol seam to a sharper native seam: `native_loading_state: native_activity_entrypoint_missing`, `native_jni_state: called`, `native_loading_library_name: libjni_latinime.so`, and `native_execute.execution_engine_ready: true`. `compatctl launch-apk --first-app-start-proof ...` now keeps that same upstream native blocker explicit through `blocking_reason: native_activity_entrypoint_missing_for_first_app_start:libjni_latinime.so`, `recommended_recovery_action: inspect_native_launch_diagnostics`, and `next_blocker: provide_native_activity_entrypoint_for_libjni_latinime_so`, while the downstream managed seam remains the stubbed `Landroid/app/Activity;->onCreate(Landroid/os/Bundle;)V` boundary once native loading is solved.

Current repo note as of `2026-05-19`: Linuxoid now also has a **Visible Wayland Interaction Checkpoint** on that same real APK path. `compatctl launch-apk --window-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` still binds the resolved `SettingsActivity` session to one deterministic `window_manager` target and records `visible_target_state`, `focus_state`, `focus_owned`, `focus_owner`, `interaction_state`, and `interaction_target_component`, but the upstream blocker it preserves is now sharper too: `blocking_reason: native_activity_entrypoint_missing:libjni_latinime.so` with `native_jni_state: called` instead of the older unresolved-symbol seam. When the host can probe a live target, Linuxoid still keeps `backing_mode`, `wayland_surface_available`, and `egl_surface_available` honest instead of pretending the app is already visibly running.

Current repo note as of `2026-05-19`: Linuxoid now also has a **Recovery and Runtime Hardening Checkpoint** on that same real APK path. Repeated `compatctl launch-apk --storage-proof ...` and `compatctl launch-apk --permissions-proof ...` runs now preserve and validate one deterministic sandbox-backed app-data, permission, and AppOps story through explicit `persisted_state_preexisting`, `continuity_validated`, `continuity_state`, and `continuity_diagnostics` fields. On the blocked real keyboard path, `compatctl launch-apk --self-heal-proof --window-proof --first-app-start-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` now keeps the earliest blocker authoritative through `primary_blocker_reason: native_activity_entrypoint_missing`, `recovery_gating_state: upstream_native_blocker_gated`, `recovery_gating_reason: native_activity_entrypoint_missing:libjni_latinime.so`, and `recommended_next_action: inspect_native_launch_diagnostics`, while downstream launch-dependent repairs are journaled as `skipped_upstream_blocker` instead of being noisily attempted behind a known native load failure.
