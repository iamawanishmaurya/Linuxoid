...
Current repo note as of `2026-05-20`: Linuxoid now also has an **Activity onCreate Dispatch Bridge Checkpoint** on that same real APK path. `compatctl launch-apk /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` still preloads the tiny Android soname compatibility shims only when staged libraries actually need them, normalizes undefined ELF version bindings when safe, gets `libjni_latinime.so` loaded, calls `JNI_OnLoad`, dispatches a real JNI registration callback, observes `RegisterNatives`, selects the real `SettingsActivity->onCreate(Landroid/os/Bundle;)V` lifecycle target, and binds a deterministic Linuxoid managed runtime-context placeholder, but it no longer stops at the broader `activity_oncreate_bundle_dispatch_required` seam, the nested `ComponentActivity` seam, the first top-level `invoke-static` seam, the first `const-string` seam after that, the first `if-nez` / `sget-boolean` / `sput-boolean` seams after that, the nested `Context.getAssets()` seam, the nested `AssetManager.open(String)` seam after that, the later no-code `Reader.read(char[])I` seam, the later `if-ltz` seam, the later `StringWriter.toString()` / inline `return-object` seam, the no-code `Closeable.close()V` seam, the no-code `String.getBytes(Charset)[B` seam, the later `array-length` seam, the later `int-to-long` seam, the later `or-long` seam, the later `const-wide/16` seam, the later `cmp-long` seam, the later `if-gtz` seam, the later `sub-long` seam, the later `add-int/2addr` seam, the later `if-ge` seam, or the nested `invoke-static/range` seam. On the real keyboard APK path Linuxoid now reports `native_loading_state: managed_activity_post_dispatch_blocked`, `native_post_dispatch_state: bytecode-blocked`, `native_post_dispatch_blocker: unsupported-dex-opcode:opcode-0x34`, `native_post_dispatch_recovery_action: extend_minimal_dex_interpreter`, `native_post_dispatch_backend: linuxoid_minimal_dex_interpreter`, `native_managed_activity_runtime_binding_state: linuxoid_runtime_context_bound`, `native_post_jni_dispatch_symbol: org.futo.inputmethod.latin.uix.settings.SettingsActivity->onCreate(Landroid/os/Bundle;)V`, and `native_execute.execution_engine_ready: true`. `compatctl launch-apk --first-app-start-proof ...` now keeps that seam explicit through `first_app_start_health: ready`, that exact `blocking_reason`, and the next concrete implementation step of extending the minimal DEX interpreter for opcode `0x34` honestly.
Current repo note as of `2026-05-20`: Linuxoid has pushed that same **Activity onCreate Dispatch Bridge Checkpoint** further down the real keyboard APK path. It now also clears `if-lt`, `if-gt`, `Thread.currentThread()`, `Thread.getId()`, `move-result-wide`, `sub-long/2addr`, `and-long/2addr`, `long-to-int`, `aget-object`, deterministic placeholder object-array length materialization, the earlier no-code `AtomicReference.getAndSet(Object)` and `AtomicReference.set(Object)` invoke targets, DEX `sub-int` (`opcode 0x91`), `rsub-int/lit16` (`opcode 0xd1`), `add-int` (`opcode 0x90`), `sub-int/2addr` (`opcode 0xb1`), `add-long/2addr` (`opcode 0xbb`), `iput-wide` (`opcode 0x5a`), the no-code `HashMap.<init>()V` plus `LinkedHashMap.<init>()V` constructor seams, `packed-switch` (`opcode 0x2b`), the no-code `Collections.singletonList(Object)` seam, DEX `instance-of` (`opcode 0x20`), the no-code `LinkedHashSet.<init>()V` seam, the no-code `Iterable.iterator()` seam, the no-code `ArrayList.<init>()V` seam, the no-code `List.size()I` seam, the no-code `List.iterator()` seam, placeholder `Iterator.next()` state propagation, the no-code `String.toCharArray()[C` seam, and the no-code `ArrayList.<init>(I)V` seam, and the exact live blocker is now `native_post_dispatch_blocker: unsupported-dex-opcode:opcode-0x49` with `native_post_dispatch_recovery_action: extend_minimal_dex_interpreter`, `native_post_dispatch_backend: linuxoid_minimal_dex_interpreter`, and `first_app_start_health: ready`. The next implementation step remains narrow and explicit: extend the minimal DEX interpreter for opcode `0x49` on the same managed bytecode path.

Current repo note as of `2026-05-20`: Linuxoid now also has a **Visible Wayland Interaction Checkpoint** on that same real APK path. `compatctl launch-apk --window-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` still binds the resolved `SettingsActivity` session to one deterministic `window_manager` target and records `visible_target_state`, `focus_state`, `focus_owned`, `focus_owner`, `interaction_state`, and `interaction_target_component`, but the upstream blocker it preserves is now sharper too: `blocking_reason: dex_invoked_method_code_item_missing:Ljava/io/Closeable;->close()V` with `native_loading_state: managed_activity_post_dispatch_blocked`, `native_post_dispatch_state: dispatch-probe-blocked`, and `native_post_dispatch_recovery_action: extend_minimal_dex_interpreter`. When the host can probe a live target, Linuxoid still keeps `backing_mode`, `wayland_surface_available`, and `egl_surface_available` honest instead of pretending the app is already visibly running.

Current repo note as of `2026-05-20`: Linuxoid now also has a **Recovery and Runtime Hardening Checkpoint** on that same real APK path. Repeated `compatctl launch-apk --storage-proof ...` and `compatctl launch-apk --permissions-proof ...` runs now preserve and validate one deterministic sandbox-backed app-data, permission, and AppOps story through explicit `persisted_state_preexisting`, `continuity_validated`, `continuity_state`, and `continuity_diagnostics` fields. On the blocked real keyboard path, `compatctl launch-apk --self-heal-proof --window-proof --first-app-start-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` now keeps the earliest blocker authoritative through `primary_blocker_reason: dex_invoked_method_code_item_missing:Ljava/io/Closeable;->close()V`, `recovery_gating_state: upstream_native_blocker_gated`, `recovery_gating_reason: dex_invoked_method_code_item_missing:Ljava/io/Closeable;->close()V`, and `recommended_next_action: extend_minimal_dex_interpreter`, while downstream launch-dependent repairs are journaled as `skipped_upstream_blocker` instead of being noisily attempted behind a known native load failure.

Current repo note as of `2026-05-20`: The next direct-runtime milestone after this checkpoint is still **P16 Managed Bytecode Invocation + ActivityThread Contract**, but it should now start from a narrower concrete blocker set: the real keyboard path no longer stops at missing receiver propagation, generic JNI registration requirements, the broader managed-dispatch-required seam, the broader managed-runtime-context-required seam, the generic `activity_oncreate_bundle_dispatch_required` seam, the nested `ComponentActivity.onCreate(Bundle)` seam, the first top-level `invoke-static` seam, the first `const-string` seam after that, the first `if-nez` / `sget-boolean` / `sput-boolean` seams after that, the nested `Context.getAssets()` seam, the nested `AssetManager.open(String)` seam, the first inline `invoke-direct` constructor seam after that, the later `InputStreamReader.<init>(InputStream, Charset)` / `BufferedReader.<init>(Reader, int)` / `StringWriter.<init>()V` seams, the placeholder `new-array` seam after that, the later no-code `Reader.read(char[])I` seam, the later `if-ltz` seam, the later `StringWriter.toString()` / inline `return-object` seam, the no-code `Closeable.close()V` seam, the no-code `String.getBytes(Charset)[B` seam, the later `array-length` seam, the later `int-to-long` seam, the later `or-long` seam, the later `const-wide/16` seam, the later `cmp-long` seam, the later `if-gtz` seam, the later `sub-long` seam, the later `add-int/2addr` seam, the later `if-ge` seam, or the nested `invoke-static/range` seam, and now stops at `unsupported-dex-opcode:opcode-0x34`. The existing sandbox-backed `package_manager`, `intent_resolution`, `activity_launch`, `storage`, `permissions`, `app_ops`, `activity_manager`, `process_manager`, `window_manager`, `runtime_bridge`, `java_apk_proof`, and `compatibility` contracts remain the real input seams. What remains blocked after this checkpoint is not manifest intake or the earlier helper-library `JNI_OnLoad` crash anymore; it is the sharper bytecode seam around a JNI-shaped `libjni_latinime.so` that loads, runs `JNI_OnLoad`, completes a real JNI registration callback, selects the real `SettingsActivity->onCreate(Landroid/os/Bundle;)V` lifecycle target, binds a deterministic `linuxoid_managed_runtime_context_placeholder`, executes the first managed post-dispatch bytecode boundary, crosses the nested `ComponentActivity.onCreate(Bundle)` seam, crosses `LayoutManager.init(Context)`, crosses `Context.getAssets()`, crosses `AssetManager.open(String)`, crosses `InputStreamReader.<init>(InputStream, Charset)`, crosses `BufferedReader.<init>(Reader, int)`, crosses `StringWriter.<init>()V`, models a placeholder array allocation, crosses the no-code `Reader.read(char[])I` boundary, crosses `if-ltz`, crosses `StringWriter.toString()` with inline `return-object`, crosses `Closeable.close()V`, crosses `String.getBytes(Charset)[B`, crosses `array-length`, crosses `int-to-long`, crosses `or-long`, crosses `const-wide/16`, crosses `cmp-long`, crosses `if-gtz`, crosses `sub-long`, crosses `add-int/2addr`, crosses `if-ge`, crosses the nested `invoke-static/range` seam, and now needs honest interpreter support for opcode `0x34` before the later ART-owned framework dispatch and ActivityThread-style application bootstrap gaps.

Current repo note as of `2026-05-19`: Linuxoid now also has a narrower synthetic checkpoint inside that same blocker. The deterministic keyboard-shaped fixture no longer collapses at missing receiver propagation inside `Landroidx/activity/ComponentActivity;->onCreate(Landroid/os/Bundle;)V`; the inline invoked-method bridge now places `p0/p1` in the real high-register parameter window, materializes placeholder object fields for the minimal `iget-object` chains it touches, stubs the saved-state and report-fragment helper calls deterministically, and returns cleanly with `framework_boundary_state: framework-stubbed` and `blocking_reason: needs-real-activitythread-context`. That means the remaining work is no longer basic invoke/register plumbing in the interpreter. It is the real managed runtime-context seam behind the live keyboard APK's `ComponentActivity.onCreate(Bundle)` path.
