...
Current repo note as of `2026-05-18`: Linuxoid now also has a **First Android App Start Checkpoint** through `compatctl launch-apk --first-app-start-proof [--package <package>] [--component <component>] <apk-path> [staging-root]` and `compatctl inspect-apk-first-start <apk-path> [staging-root]`. That checkpoint reuses the real direct-session package, activity, process, window, runtime, and Java-proof seams, persists `sandbox/data/data/<package>/first-app-start/first-app-start.json`, and reports whether Linuxoid actually executed Java/ART bytecode or stopped at the exact `needs-real-art-execution` boundary. The healthy fixture path now reaches the real managed-runtime handoff honestly, but full ART-owned bytecode invocation is still blocked.

Current repo note as of `2026-05-18`: The next direct-runtime milestone after P15 is **P16 Managed Bytecode Invocation + ActivityThread Contract**. P16 should consume the existing sandbox-backed `package_manager`, `intent_resolution`, `activity_launch`, `storage`, `permissions`, `app_ops`, `activity_manager`, `process_manager`, `window_manager`, `runtime_bridge`, `java_apk_proof`, and `compatibility` contracts plus the current Self-Healing Android Device health gates (`storage_health`, `sandbox_health`, `permission_health`, `app_ops_health`, `binder_health`, `dex_health`, `activity_health`, `activity_manager_health`, `process_health`, `window_health`, `runtime_health`, and `java_proof_health`) as its real managed-runtime-start inputs. What remains blocked after P15 is real ART-owned class loading, ActivityThread-style application bootstrap, managed bytecode invocation, richer framework-level permission/AppOps enforcement, and full Java/Kotlin execution across broader third-party APKs.
