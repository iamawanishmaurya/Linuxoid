...
Current repo note as of `2026-05-19`: Linuxoid now also has a sharpened **Native App-Start Bridge Checkpoint** on that same real APK path. `compatctl launch-apk /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` still preloads the tiny Android soname compatibility shims only when staged libraries actually need them, normalizes undefined ELF version bindings when safe, gets `libjni_latinime.so` loaded, and calls `JNI_OnLoad`, but it no longer collapses that success back into a generic missing-entrypoint story. On the real keyboard APK path Linuxoid now reports `native_loading_state: linuxoid_managed_app_start_bridge_required`, `native_jni_state: called`, `native_app_start_bridge_state: linuxoid_managed_app_start_bridge_required`, `native_post_jni_startup_state: managed_activity_dispatch_required`, `native_loading_library_name: libjni_latinime.so`, and `native_execute.execution_engine_ready: true`. `compatctl launch-apk --first-app-start-proof ...` now keeps that same upstream seam explicit through `blocking_reason: linuxoid_managed_app_start_bridge_required_for_first_app_start:libjni_latinime.so`, `recommended_recovery_action: inspect_native_launch_diagnostics`, and `next_blocker: implement_linuxoid_managed_app_start_bridge_for_libjni_latinime_so`, while the downstream managed seam remains the stubbed `Landroid/app/Activity;->onCreate(Landroid/os/Bundle;)V` boundary once Linuxoid owns the managed app-start bridge.

Current repo note as of `2026-05-19`: Linuxoid now also has a **Visible Wayland Interaction Checkpoint** on that same real APK path. `compatctl launch-apk --window-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` still binds the resolved `SettingsActivity` session to one deterministic `window_manager` target and records `visible_target_state`, `focus_state`, `focus_owned`, `focus_owner`, `interaction_state`, and `interaction_target_component`, but the upstream blocker it preserves is now sharper too: `blocking_reason: linuxoid_managed_app_start_bridge_required:libjni_latinime.so` with `native_jni_state: called` and `native_post_jni_startup_state: managed_activity_dispatch_required`. When the host can probe a live target, Linuxoid still keeps `backing_mode`, `wayland_surface_available`, and `egl_surface_available` honest instead of pretending the app is already visibly running.
