...
Current repo note as of `2026-05-18`: Linuxoid now also has a **JNI and Native Loading Checkpoint** on that same real APK path. `compatctl launch-apk /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` still fails honestly when a staged host-ABI library cannot load, but it no longer hides behind only `libraries_failed_to_load`. The report now preserves deterministic `native_execute.library_load_attempts[]` evidence for each staged host-ABI library and surfaces exact top-level `native_loading_state`, `native_jni_state`, `native_loading_library_name`, `native_loading_library_path`, and `native_loading_detail` fields. `compatctl launch-apk --first-app-start-proof ...` now keeps that same upstream native blocker explicit through `blocking_reason: native_dlopen_failed_for_first_app_start:<library>`, `recommended_recovery_action: inspect_native_launch_diagnostics`, and an exact next blocker, while the downstream managed seam remains the stubbed `Landroid/app/Activity;->onCreate(Landroid/os/Bundle;)V` boundary once native loading is solved.

Current repo note as of `2026-05-18`: Linuxoid now also has a **Visible Wayland Interaction Checkpoint** on that same real APK path. `compatctl launch-apk --window-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` now binds the resolved `SettingsActivity` session to one deterministic `window_manager` target, records `visible_target_state`, `focus_state`, `focus_owned`, `focus_owner`, `interaction_state`, and `interaction_target_component`, and keeps the real upstream native blocker explicit through `blocking_reason: native_dlopen_failed:libandroidx.graphics.path.so` plus `recommended_recovery_action: inspect_native_launch_diagnostics`. When the host can probe a live target, Linuxoid also keeps `backing_mode`, `wayland_surface_available`, and `egl_surface_available` honest instead of pretending the app is already visibly running.

Current repo note as of `2026-05-19`: Linuxoid now also has a **Recovery and Runtime Hardening Checkpoint** on that same real APK path. Repeated `compatctl launch-apk --storage-proof ...` and `compatctl launch-apk --permissions-proof ...` runs now preserve and validate one deterministic sandbox-backed app-data, permission, and AppOps story through explicit `persisted_state_preexisting`, `continuity_validated`, `continuity_state`, and `continuity_diagnostics` fields. On the blocked real keyboard path, `compatctl launch-apk --self-heal-proof --window-proof --first-app-start-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` now keeps the earliest blocker authoritative through `primary_blocker_reason: native_dlopen_failed:libandroidx.graphics.path.so`, `recovery_gating_state: upstream_native_blocker_gated`, `recovery_gating_reason: native_dlopen_failed:libandroidx.graphics.path.so`, and `recommended_next_action: inspect_native_launch_diagnostics`, while downstream launch-dependent repairs are journaled as `skipped_upstream_blocker` instead of being noisily attempted behind a known native load failure.
