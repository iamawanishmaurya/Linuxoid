...
Current repo note as of `2026-05-19`: Linuxoid now also has a **Managed Runtime Context Binding Checkpoint** on that same real APK path. `compatctl launch-apk /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` still preloads the tiny Android soname compatibility shims only when staged libraries actually need them, normalizes undefined ELF version bindings when safe, gets `libjni_latinime.so` loaded, calls `JNI_OnLoad`, dispatches a real JNI registration callback, and observes `RegisterNatives`, but it no longer stops at the broader managed-runtime-context-required seam. On the real keyboard APK path Linuxoid now reports `native_loading_state: activity_oncreate_bundle_dispatch_required`, `native_jni_state: called`, `native_app_start_bridge_state: linuxoid_managed_app_start_bridge_selected`, `native_registration_dispatch_state: called`, `native_registration_dispatch_symbol_kind: registration_callback`, `native_registration_dispatch_symbol: _ZN8latinime22register_LanguageModelEP7_JNIEnv`, `native_registration_outcome_state: register_natives_completed`, `native_managed_activity_dispatch_state: linuxoid_dispatch_attempted`, `native_managed_activity_dispatch_component: org.futo.inputmethod.latin/.uix.settings.SettingsActivity`, `native_managed_activity_runtime_binding_state: linuxoid_runtime_context_bound`, `native_managed_activity_runtime_binding_reason: managed_activity_dispatch_target_bound_to_linuxoid_runtime_context`, `native_managed_activity_runtime_context_kind: linuxoid_managed_runtime_context_placeholder`, `native_post_jni_dispatch_symbol_kind: managed_activity_lifecycle_method`, `native_post_jni_dispatch_symbol: org.futo.inputmethod.latin.uix.settings.SettingsActivity->onCreate(Landroid/os/Bundle;)V`, `native_post_jni_startup_state: activity_oncreate_bundle_dispatch_required`, `native_loading_library_name: libjni_latinime.so`, and `native_execute.execution_engine_ready: true`. `compatctl launch-apk --first-app-start-proof ...` now keeps that same upstream seam explicit through `blocking_reason: activity_oncreate_bundle_dispatch_required_for_first_app_start:libjni_latinime.so`, `recommended_recovery_action: inspect_native_launch_diagnostics`, and `next_blocker: bridge_activity_oncreate_bundle_dispatch_into_managed_runtime_context`, while the downstream managed seam remains the real `Activity.onCreate(Bundle)` dispatch bridge once Linuxoid owns more than the deterministic runtime-context placeholder.

Current repo note as of `2026-05-19`: Linuxoid now also has a **Visible Wayland Interaction Checkpoint** on that same real APK path. `compatctl launch-apk --window-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` still binds the resolved `SettingsActivity` session to one deterministic `window_manager` target and records `visible_target_state`, `focus_state`, `focus_owned`, `focus_owner`, `interaction_state`, and `interaction_target_component`, but the upstream blocker it preserves is now sharper too: `blocking_reason: activity_oncreate_bundle_dispatch_required:libjni_latinime.so` with `native_jni_state: called`, `native_managed_activity_dispatch_state: linuxoid_dispatch_attempted`, `native_managed_activity_runtime_binding_state: linuxoid_runtime_context_bound`, and `native_post_jni_startup_state: activity_oncreate_bundle_dispatch_required`. When the host can probe a live target, Linuxoid still keeps `backing_mode`, `wayland_surface_available`, and `egl_surface_available` honest instead of pretending the app is already visibly running.

Current repo note as of `2026-05-19`: Linuxoid now also has a **Recovery and Runtime Hardening Checkpoint** on that same real APK path. Repeated `compatctl launch-apk --storage-proof ...` and `compatctl launch-apk --permissions-proof ...` runs now preserve and validate one deterministic sandbox-backed app-data, permission, and AppOps story through explicit `persisted_state_preexisting`, `continuity_validated`, `continuity_state`, and `continuity_diagnostics` fields. On the blocked real keyboard path, `compatctl launch-apk --self-heal-proof --window-proof --first-app-start-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity /home/astra/Downloads/keyboard-0.1.28.apk [staging-root]` now keeps the earliest blocker authoritative through `primary_blocker_reason: activity_oncreate_bundle_dispatch_required:libjni_latinime.so`, `recovery_gating_state: upstream_native_blocker_gated`, `recovery_gating_reason: activity_oncreate_bundle_dispatch_required:libjni_latinime.so`, and `recommended_next_action: inspect_native_launch_diagnostics`, while downstream launch-dependent repairs are journaled as `skipped_upstream_blocker` instead of being noisily attempted behind a known native load failure.

Current repo note as of `2026-05-19`: The next direct-runtime milestone after this checkpoint is still **P16 Managed Bytecode Invocation + ActivityThread Contract**, but it should now start from a narrower concrete blocker set: the real keyboard path no longer stops at missing receiver propagation, generic JNI registration requirements, the broader managed-dispatch-required seam, or the broader managed-runtime-context-required seam, and now stops at `activity_oncreate_bundle_dispatch_required` with `next_blocker: bridge_activity_oncreate_bundle_dispatch_into_managed_runtime_context`. The existing sandbox-backed `package_manager`, `intent_resolution`, `activity_launch`, `storage`, `permissions`, `app_ops`, `activity_manager`, `process_manager`, `window_manager`, `runtime_bridge`, `java_apk_proof`, and `compatibility` contracts remain the real input seams. What remains blocked after this checkpoint is not manifest intake or the earlier helper-library `JNI_OnLoad` crash anymore; it is the sharper seam around a JNI-shaped `libjni_latinime.so` that loads, runs `JNI_OnLoad`, completes a real JNI registration callback, selects the real `SettingsActivity->onCreate(Landroid/os/Bundle;)V` lifecycle target, binds a deterministic `linuxoid_managed_runtime_context_placeholder`, and still needs real managed `Activity.onCreate(Bundle)` dispatch plus the later real ART-owned framework dispatch and ActivityThread-style application bootstrap gaps.
