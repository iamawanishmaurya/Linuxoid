...
Linuxoid now also exposes a **First Android App Start Checkpoint** through `launch-apk --first-app-start-proof [--package <package>] [--component <component>] <apk-path> [staging-root]` and `inspect-apk-first-start <apk-path> [staging-root]`. That focused checkpoint does not add a separate mock layer. It reuses the real direct-session package, activity, process, window, runtime, and Java-proof bridges, persists `sandbox/data/data/<package>/first-app-start/first-app-start.json`, and answers the hard question directly: did Linuxoid actually execute Java/ART bytecode for the resolved `MainActivity`, or did it stop at the exact managed-runtime boundary? On the healthy deterministic fixture path today, the answer is still honest: Linuxoid reaches `needs-real-art-execution`, reports `java_art_bytecode_executed: false`, and names the next blocker as `implement_real_art_activity_bytecode_invocation`.
