...
Linuxoid now also exposes a **First MainActivity Bytecode Checkpoint** through `launch-apk --first-app-start-proof [--package <package>] [--component <component>] <apk-path> [staging-root]` and `inspect-apk-first-start <apk-path> [staging-root]`. That focused checkpoint does not add a separate mock layer. It reuses the real direct-session package, activity, process, window, runtime, and Java-proof bridges, persists `sandbox/data/data/<package>/first-app-start/first-app-start.json`, parses real DEX string plus type plus proto plus method plus class tables, and answers the harder question directly: did Linuxoid actually decode and execute a real DEX instruction path for the resolved `MainActivity`, or did it stop at an exact unsupported or runtime-handoff boundary? On the healthy deterministic fixture path today, Linuxoid executes the fixture `com.example.launchapk.MainActivity.onCreate()I` method through its minimal interpreter, executes `const/4` followed by `return`, reports `java_art_bytecode_executed: true`, reports the returned integer value, reports `blocking_reason: needs-real-activitythread-context`, and names the next blocker as `bridge_activity_oncreate_into_real_art_runtime_context`. That is still not full ART-owned Java/Kotlin application execution yet.
