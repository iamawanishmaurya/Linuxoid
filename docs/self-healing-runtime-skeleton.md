...
- Linuxoid now also has enough direct-session contract surface to hand off cleanly into **P12 Runtime Process Handoff + Resume Contract**:
  ...
  - `activity_manager`
  - `process_manager`
  - Self-Healing Android Device health gates for storage, sandbox, permission, AppOps, Binder, DEX, activity, activity-manager, and process readiness
...
- real host-side process ownership and liveness beyond the current Linuxoid placeholder process identity
