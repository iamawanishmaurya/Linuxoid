...
Linuxoid still does **not** launch `/home/astra/Downloads/keyboard-0.1.28.apk` as a working app on Linux yet. The current narrow native blocker is no longer the earlier helper-library `JNI_OnLoad` crash or the older registration-required seam. After the Android-compat preload slice, deferred `JNI_OnLoad` selection logic, and Linuxoid-owned JNI registration dispatch, the real keyboard path now reports:
...
- `native_post_jni_startup_state: activity_oncreate_bundle_dispatch_required`
- `native_post_jni_dispatch_symbol_kind: managed_activity_lifecycle_method`
...
- `native_post_dispatch_state: bytecode-blocked`
- `native_post_dispatch_blocker: unsupported-dex-opcode:opcode-0x49`
- `native_post_dispatch_recovery_action: extend_minimal_dex_interpreter`
...
That is progress, but it is still not a working launch. Linuxoid now gets the real entry library loaded, calls `JNI_OnLoad`, dispatches a real registration callback, observes `RegisterNatives`, selects the real `SettingsActivity->onCreate(Landroid/os/Bundle;)V` lifecycle target, binds a deterministic Linuxoid managed runtime-context placeholder for that target, resolves the staged DEX entrypoint, crosses the nested `ComponentActivity.onCreate(Bundle)` seam, crosses the earlier top-level `LayoutManager.init(Context)` call boundary, crosses `Context.getAssets()`, crosses `AssetManager.open(String)`, crosses `InputStreamReader.<init>(InputStream, Charset)`, crosses `BufferedReader.<init>(Reader, int)`, crosses `StringWriter.<init>()V`, models one placeholder `new-array` allocation, crosses the earlier no-code `Reader.read(char[])I` seam, crosses the earlier `if-ltz` and inline `return-object` seams, crosses `StringWriter.toString()`, crosses `Closeable.close()V`, crosses `String.getBytes(Charset)[B`, crosses `array-length`, crosses `int-to-long`, crosses `or-long`, crosses `const-wide/16`, crosses `cmp-long`, crosses `if-gtz`, crosses `sub-long`, crosses `add-int/2addr`, crosses `if-ge`, crosses the nested `invoke-static/range` seam, crosses `if-lt`, `if-gt`, `Thread.currentThread()`, `Thread.getId()`, `move-result-wide`, `sub-long/2addr`, `and-long/2addr`, `long-to-int`, `aget-object`, deterministic placeholder object-array length materialization, the earlier no-code `AtomicReference.getAndSet(Object)` and `AtomicReference.set(Object)` seams, DEX `sub-int` (`opcode 0x91`), `rsub-int/lit16` (`opcode 0xd1`), `add-int` (`opcode 0x90`), `sub-int/2addr` (`opcode 0xb1`), `add-long/2addr` (`opcode 0xbb`), `iput-wide` (`opcode 0x5a`), `HashMap.<init>()V`, `LinkedHashMap.<init>()V`, `packed-switch` (`opcode 0x2b`), the no-code `Collections.singletonList(Object)` seam, DEX `instance-of` (`opcode 0x20`), the no-code `LinkedHashSet.<init>()V` seam, the no-code `Iterable.iterator()` seam, the no-code `ArrayList.<init>()V` seam, the no-code `List.size()I` seam, the no-code `List.iterator()` seam, placeholder `Iterator.next()` state propagation, the no-code `String.toCharArray()[C` seam, and the no-code `ArrayList.<init>(I)V` seam, and now stops at the honest next blocker: `unsupported-dex-opcode:opcode-0x49`. The managed checkpoint remains honest too: `launch-apk --first-app-start-proof --package org.futo.inputmethod.latin --component org.futo.inputmethod.latin/.uix.settings.SettingsActivity ...` now reports that exact `blocking_reason`, `first_app_start_health: ready`, and the next concrete implementation step is extending the minimal DEX interpreter for opcode `0x49` without pretending a broader ART or framework story.

Linuxoid also now keeps that blocker stable on keyboard-shaped long staging roots by hardening the local Binder-shaped transport simulation, and it now crosses the earlier `const-string` seam instead of stopping there. The first-app-start path no longer dies early at `unable to write binder transport payload`, and it no longer stops at the first top-level `invoke-static` seam after `ComponentActivity.onCreate(Bundle)` returns.
...
### Managed Activity Start Checkpoint

`launch-apk --first-app-start-proof` and `inspect-apk-first-start` now expose the first real managed-activity start seam on top of the existing direct Linuxoid session path:
...
- Linuxoid still resolves and executes the deterministic fixture `com.example.launchapk.MainActivity.onCreate()I`
...
- Linuxoid materializes a deterministic lifecycle receiver placeholder for `MainActivity` and binds it to register `v0`
- Linuxoid crosses the first framework-style call boundary as a stubbed `Landroid/app/Activity;->onCreate()V`
...
- Linuxoid now also executes one deeper keyboard-shaped synthetic seam by crossing `Landroidx/activity/ComponentActivity;->onCreate(Landroid/os/Bundle;)V` through the inline invoked-method bridge, materializing placeholder object fields for the minimal `iget-object` path it needs, stubbing the saved-state and lifecycle helper calls deterministically, and returning cleanly to the caller
...
- `first_android_app_start.invoked_method_class_descriptor: "Landroid/app/Activity;"`
- `first_android_app_start.invoked_method_name: "onCreate"`
- `first_android_app_start.invoked_method_signature: "()V"`
...
- `first_android_app_start.lifecycle_receiver_state: "receiver-placeholder-materialized"`
...
- `first_android_app_start.dex_parse_state: "entrypoint_code_item_resolved"`
...
- `first_android_app_start.field_name: "value"`
...
On the real keyboard verification path today, Linuxoid now reports:

- `first_android_app_start.activity_name: "org.futo.inputmethod.latin.uix.settings.SettingsActivity"`
- `first_android_app_start.activity_component: "org.futo.inputmethod.latin/.uix.settings.SettingsActivity"`
- `first_android_app_start.activity_target_resolution_state: "resolved-from-intent-contract"`
- `first_android_app_start.entrypoint_class_descriptor: "Lorg/futo/inputmethod/latin/uix/settings/SettingsActivity;"`
- `first_android_app_start.class_loading_state: "resolved-from-staged-dex"`
...
So this is a truthful managed-activity start checkpoint for the Self-Healing Android Device path. Linuxoid now resolves the real keyboard `SettingsActivity` class and `onCreate(Bundle)V` seam from staged DEX metadata, materializes the lifecycle receiver and Bundle placeholders, executes one real `invoke-super` bytecode boundary, crosses the earlier top-level `invoke-static` seam, crosses `Context.getAssets()`, crosses `AssetManager.open(String)`, crosses `InputStreamReader.<init>(InputStream, Charset)`, crosses `BufferedReader.<init>(Reader, int)`, crosses `StringWriter.<init>()V`, models one placeholder array allocation, crosses the earlier no-code `Reader.read(char[])I` boundary, crosses the earlier `if-ltz` branch seam, crosses inline `StringWriter.toString()` with `return-object`, crosses `Closeable.close()V`, crosses `String.getBytes(Charset)[B`, crosses `array-length`, crosses `int-to-long`, crosses `or-long`, crosses `const-wide/16`, crosses `cmp-long`, crosses `if-gtz`, crosses `sub-long`, crosses `add-int/2addr`, crosses `if-ge`, crosses the nested `invoke-static/range` seam, and now reports the exact next interpreter seam `unsupported-dex-opcode:opcode-0x34` instead of collapsing back to a generic native blocker. The deterministic execution fixture still proves the minimal interpreter can cross one framework boundary, constructor seams, one object-reference field round-trip, one static-boolean write/read path, one `const-string` materialization to a real `return`, one nested `invoke-static -> invoke-direct` helper seam, one placeholder `new-array` seam, one no-code `Reader.read(char[])I` stub seam, one `if-ltz` branch seam, one `if-gtz` branch seam, one `if-ge` branch seam, one inline `return-object` seam via `StringWriter.toString()`, one no-code `Closeable.close()V` seam, one no-code `String.getBytes(Charset)[B` seam, one nested `invoke-static/range` seam, one `or-long` seam, one `const-wide/16` seam, one `cmp-long` seam, one `sub-long` seam, and one `add-int/2addr` seam. That is still not a claim that Linuxoid already provides a real ART class loader, ActivityThread, heap, or end-to-end Android framework execution.

Linuxoid now also proves one narrower synthetic seam inside that same area: the deterministic keyboard-shaped fixture reaches `Landroidx/activity/ComponentActivity;->onCreate(Landroid/os/Bundle;)V`, seeds `p0/p1` in the correct high-register parameter window, materializes placeholder object fields when the inline method first touches saved-state or context-aware helper members, stubs the minimal AndroidX helper calls it needs, and returns with `framework_boundary_state: "framework-stubbed"` plus `framework_boundary_reason: "placeholder_object_field_materialized_for_minimal_checkpoint"`. That is still an interpreter-owned checkpoint, not real ART heap or ActivityThread execution.

Immediate next blocker:

- real keyboard APK end-to-end launch now blocks at `managed_activity_post_dispatch_blocked`
- Linuxoid now reports that upstream blocker as the exact post-dispatch seam `unsupported-dex-opcode:opcode-0x34`
...
- the next concrete interpreter step is extending the minimal DEX interpreter for opcode `0x34`
